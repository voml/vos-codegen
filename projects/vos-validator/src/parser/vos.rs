...
// Hash of the grammar file: EDA5AE22F92D642A9D3AA93E270868F0956A05BC509597D14D741F3C9E8C3A4F
...
mod peginator_generated {
    ...
    impl PegPosition for DeclareBodyNode {
        fn position(&self) -> &std::ops::Range<usize> {
            match self {
                ...
                Self::KeyValueNode(x) => x.position(),
    ...
    mod KeyValueNode_impl {
        ...
        pub fn rule_parser<'a>(
            ...
            let result = parse(state.clone(), tracer, cache)?.map_with_state(|r, new_state| super::KeyValueNode {
                key: r.key,
                value: r.value,
                position: state.range_until(new_state),
            });
    ...
    impl PegPosition for KeyValueNode {
        fn position(&self) -> &std::ops::Range<usize> {
            &self.position
        }
    }
