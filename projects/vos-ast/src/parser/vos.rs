...
// Hash of the grammar file: D624CB7091266E25C05BC841B6A06B7ECA664AC342CEAB4B6D6CEE203456C3F4
...
mod peginator_generated {
    ...
    mod DeclareBodyNode_impl {
        ...
        pub fn parse<'a>(
            ...
            let mut iterations: usize = 0;
            let mut state = state;
            let mut pairs: Vec<KeyValueNode> = Vec::new();
            loop {
                match parse_Whitespace(state.clone(), tracer, cache)
        ...
        pub fn rule_parser<'a>(
            ...
            let result = parse(state.clone(), tracer, cache)?
                .map_with_state(|r, new_state| super::DeclareBodyNode { pairs: r, position: state.range_until(new_state) });
            Ok(result)
        }
    }
    #[inline]
